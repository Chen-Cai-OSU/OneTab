...
class OneTabManager:
    ...
    def setup_ui(self):
        ...
        ttk.Button(file_frame, text="Load OneTab Data", command=self.load_file).pack(side=tk.LEFT, padx=5)
        ttk.Button(file_frame, text="Save Filtered Data", command=self.save_filtered).pack(side=tk.LEFT, padx=5)
        ttk.Button(file_frame, text="Export as JSON", command=self.export_json).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(search_frame, text="Clear Search", command=self.clear_search).pack(side=tk.LEFT, padx=5)
        
        # Selection operations
        ttk.Button(search_frame, text="Select All Visible", command=self.select_all_visible).pack(side=tk.LEFT, padx=20)
        ttk.Button(search_frame, text="Deselect All", command=self.deselect_all).pack(side=tk.LEFT, padx=5)
        ttk.Button(search_frame, text="Delete Selected", command=self.delete_selected).pack(side=tk.LEFT, padx=5)
        ...
        style = ttk.Style()
        style.configure("Treeview", selectbackground="lightblue", selectforeground="black")
        ...
        self.info_label = ttk.Label(info_frame, text="Total: 0 tabs | Filtered: 0 tabs | Selected: 0 tabs")
        self.info_label.pack(side=tk.LEFT)
        
        # Keyboard shortcuts info
        shortcuts_label = ttk.Label(info_frame, text="Shortcuts: Ctrl+A (Select All) | Delete (Delete Selected) | Ctrl+F (Search)", 
                                   foreground="gray")
    ...
    def update_info(self):
        ...
        self.info_label.config(text=f"Total: {total} tabs | Filtered: {filtered} tabs | Selected: {selected} tabs")
    ...
    def on_search_changed(self, event=None):
        ...
        if not search_text:
            ...
            for tab in self.tabs_data:
                if (search_text in tab['title'].lower() or 
                    search_text in tab['url'].lower() or
                    search_text in tab['domain'].lower()):
    ...
    def save_filtered(self):
        ...
        if filename:
            try:
                ...
                messagebox.showinfo("Success", f"Saved {len(self.tabs_data)} tabs to {os.path.basename(filename)}")
    ...
    def export_json(self):
        ...
        if filename:
            try:
                ...
                messagebox.showinfo("Success", f"Exported {len(self.tabs_data)} tabs to {os.path.basename(filename)}")
